...
function populateAdminForm() {
  ...
  $('#inputProductDetails').value = JSON.stringify(content.productDetails || [], null, 2);
...
$('#addLookbookBtn').addEventListener('click', async () => {
  ...
  const tags = normalizeTags($('#quickLookbookTags').value);
  ...
  existing.unshift({ title, image, description, tags });
...
$('#saveBtn').addEventListener('click', () => {
  try {
    ...
    content.productDetails = JSON.parse($('#inputProductDetails').value);
