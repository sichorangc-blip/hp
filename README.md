...
## 제품 상세페이지 구성
- 메인의 `제품 상세 페이지` 섹션에서 각 제품 카드의 `상세페이지 보기`를 누르면 `product.html?id=<제품id>`로 이동합니다.
- 관리자 페이지의 `제품 상세페이지 JSON`에서 제품 설명/특징/이미지를 편집할 수 있습니다.

## 해시태그 교집합 이미지 로드
- 룩북 모드를 `해시태그 교집합 이미지 로드 모드`로 선택하면,
  입력한 해시태그를 **모두 포함**한(`AND` 조건) 룩북 이미지들만 표시됩니다.
- 룩북 JSON 각 항목에 `tags` 배열을 넣어 주세요. 예: `"tags": ["iset", "daily"]`
- 인스타그램 외부 정책/CORS 제한으로 공개 웹에서 해시태그를 조합해 원본 이미지를 자동 수집하는 방식은 제한될 수 있습니다.
  그래서 실사용 가능한 안정 방식으로 교집합 필터 렌더링을 제공합니다.

- 관리자 비밀번호를 잊었을 때: `admin.html` 로그인 창의 `비밀번호 초기화` 버튼으로 초기값(`iset2026!`) 복구 가능

- 로그인 복구 안전장치: 저장된 커스텀 비밀번호를 잊어도 기본 비밀번호 `iset2026!` 입력 시 자동 복구 로그인됩니다.
